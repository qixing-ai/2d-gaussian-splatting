...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if intr.model=="SIMPLE_PINHOLE":
            ...
            cy = intr.params[2]
            # k 畸变参数
            distortion_params = np.array([intr.params[3]])
            FovY = focal2fov(focal_length_x, height)
            FovX = focal2fov(focal_length_x, width)
            ...
            cy = intr.params[2]
            # k1, k2 畸变参数
            distortion_params = np.array([intr.params[3], intr.params[4]])
            FovY = focal2fov(focal_length_x, height)
            FovX = focal2fov(focal_length_x, width)
        else:
            assert False, "Colmap camera model not handled: only PINHOLE, SIMPLE_PINHOLE, SIMPLE_RADIAL, or RADIAL cameras supported!"
